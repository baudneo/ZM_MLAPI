...
from zm_mlapi.imports import ModelProcessor, ALPRModelOptions, MLModelConfig, ALPRModelConfig, ALPRService
...
class AlprBase:
    def __init__(self, model_config: ALPRModelConfig, model_options: ALPRModelOptions):
        if not model_config:
            raise ValueError(f"{LP} no config passed!")
        # Model init params
        self.config: ALPRModelConfig = model_config
        self.options: ALPRModelOptions = model_options
        self.processor: ModelProcessor = self.options.processor
        self.model_name = self.config.name
        if not self.config.alpr_key and self.config.alpr_service == ALPRService.CLOUD:
            logger.debug(f"{LP} API key not specified and you are not using the command line ALPR, did you forget?")
        self.api_key = self.config.alpr_key
        import tempfile
        self.tempdir = tempfile.gettempdir()
        self.url = self.config.alpr_url
        self.filename = None
        ...
        if files:
            for file in files:
                os.remove(file)
        logger.debug(f"{LP} INITIALIZING '{self.model_name}' with options -> {self.options}")
    ...
    def set_api_key(self, key):
        self.api_key = key
        logger.debug(f"{LP} set_key-> key changed")

    def stats(self):
        logger.warning(f"{LP} stats not implemented in base class")

    def detect(self, input_image=None):
        logger.warning(f"{LP} detect not implemented in base class")

    def prepare(self, alpr_object):
        if not isinstance(alpr_object, str):
            logger.debug(f"{LP} the supplied object is not an absolute file path, assuming blob and creating file")
            if self.options.get("max_size"):
                logger.debug(2, f"{LP} resizing image blob using max_size={self.options.get('max_size')}")
            ...
            logger.debug(2, f"{LP} the supplied object is an absolute file path -> '{alpr_object}'")
...
class Alpr(AlprBase):
    def __init__(self, options=None, tempdir="/tmp", **kwargs):
        ...
        """
        super(Alpr, self).__init__(options=options, tempdir=tempdir)
        if options is None:
            options = {}
        self.alpr_obj = None
...
class PlateRecognizer(AlprBase):
    ...
    def __init__(self, options: Optional[ALPRModelOptions]):
        """Wrapper class for platerecognizer.com

        """
        super(PlateRecognizer, self).__init__(options=options)
        self.url = "https://api.platerecognizer.com/v1"
        logger.debug(f"{LP}plate rec: initialized with url: {self.url}")
    ...
    def stats(self):
        ...
        if self.options.get("alpr_api_type") != "cloud":
            logger.debug(f"{LP}plate rec: local SDK does not provide stats")
            return {}
        try:
            if self.api_key:
    ...
    def detect(self, input_image=None):
        ...
        if str2bool(self.options.get("platerec_stats")):
            logger.debug(2, f"{LP}plate rec:  API usage stats: {json.dumps(self.stats())}")
        with open(self.filename, "rb") as fp:
            try:
                ...
                if self.options.get("platerec_config"):
                    logger.debug("{LP}plate rec: found platerec_config, using it")
                ...
                c = response.content
                response = {"error": f"Plate recognizer rejected the upload with: {e}.", "results": []}
                logger.error(f"{LP}plate rec: API rejected the upload with {e} and body:{c}")
            else:
                response = response.json()
                logger.debug(3, f"{LP}plate rec: API response JSON={response}")
        ...
        if len(labels):
            logger.debug(2, f"{LP}plate rec: Exiting ALPR with labels: {labels}")
        else:
            logger.debug(2, f"{LP}plate rec: Exiting ALPR with nothing detected")
...
class OpenAlpr(AlprBase):
    def __init__(self, options=None, tempdir="/tmp"):
        ...
        logger.debug(f"{LP} OpenALPR initialized with url: {self.url}")
    ...
    def detect(self, input_image=None):
        ...
        with Path(self.filename).open("rb") as fp:
            try:
                ...
                rurl = f"{self.url}?secret_key={self.api_key}{params}"
                logger.debug(2, f"Trying OpenALPR with url:{rurl}")
                response = requests.post(rurl, files={"image": fp})
                fp.close()
                response.raise_for_status()
            except requests.exceptions.RequestException as e:
                response = {"error": f"Open ALPR rejected the upload with {e}", "results": []}
                logger.debug(f"Open ALPR rejected the upload with {e}")
            else:
                response = response.json()
                logger.debug(2, f"OpenALPR JSON: {response}")
...
class OpenAlprCmdLine(AlprBase):
    def __init__(self, options=None, tempdir="/tmp"):
        ...
        if self.cmd.lower().find("-j") == -1:
            logger.debug(2, "{LP}cmdline: Adding -j to OpenALPR for JSON output")
    ...
    def detect(self, input_image=None):
        ...
        logger.debug(2, f"{LP}cmdline: executing: '{do_cmd}'")
        ...
        logger.debug(2, f"perf:{LP}cmdline: took {diff_time}")
        logger.debug(2, f"{LP}cmdline: JSON response -> {response.decode('utf8')}")
        try:
            response = json.loads(response)
        except ValueError as e:
            logger.error(f"{LP}cmdline: Error parsing JSON response -> {e}")
            response = {}
